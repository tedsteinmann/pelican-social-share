...
def capture_social_cards(pelican_obj: Any) -> None:
    ...
    if settings.get("SOCIAL_DISABLE_SCREENSHOT", False):
        pelican_obj.logger.info("[social_share] Screenshots disabled")
        return
    
    if not PLAYWRIGHT_AVAILABLE:
        pelican_obj.logger.warning(
    ...
    if not os.path.isdir(social_html_dir):
        pelican_obj.logger.debug("[social_share] No social HTML directory found")
        return
    ...
    if not social_pages:
        pelican_obj.logger.debug("[social_share] No social pages to process")
        return

    # Collect taglines for hash calculation
    taglines = {}
    for generator in pelican_obj.generators:
        if hasattr(generator, "articles"):
            for article in generator.articles:
                if article.slug and article.metadata.get("tagline"):
                    taglines[article.slug] = article.metadata["tagline"]
        if hasattr(generator, "pages"):
            for page in generator.pages:
                if page.slug and page.metadata.get("tagline"):
                    taglines[page.slug] = page.metadata["tagline"]
